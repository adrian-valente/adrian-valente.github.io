...
# How it works
As we mentioned, CCA takes as input two linked sets of vectors $$\{x_i\}_{1\leq i \leq N}$$ and $$\{\mathbf{y}_i\}_{1\leq i \leq N}$$, with the $$\mathbf{x}$$'s and $$\mathbf{y}$$'s of two not necessarily equal dimensionalities $$d_1$$ and $$d_2$$. We can put them into two data matrices $$\mathbf{X} \in \mathbb{R}^{N \times d_1}$$ and $$\mathbf{Y} \in \mathbb{R}^{N \times d_2}$$. Note that contrarily to the dimensionality, the number of samples $$N$$ has to be equal accross the two datasets (as is the case for Pearson's correlation coefficient as well). Also note in everything that follows I will considered data has been centered (mean of $$\mathbf{x}$$ and $$\mathbf{y}$$ is 0 on each coordinate).
